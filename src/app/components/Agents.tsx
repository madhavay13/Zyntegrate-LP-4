  ...
  useEffect(() => {
    const runSequence = async () => {
      while (true) {
        ...
        // Animate workflow steps with delayed tick
  ...
  const rightSideCards = [
    {
      ...
      description: (steps: string[]) => `Detects "${steps[0]}" and other incoming events.`,
    },
    {
      label: "Trigger",
      title: (steps: string[]) => steps[1] || "",
      description: (steps: string[]) => `Automatically triggers "${steps[1]}" in connected systems.`,
    },
    {
      label: "Notifications & Monitoring",
      title: (steps: string[]) => steps[2] || "",
      description: (steps: string[]) => `Executes "${steps[2]}" and keeps track of workflow progress.`,
    },
  ];
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10 grid lg:grid-cols-2 gap-10 items-stretch">
        ...
        <div ref={scope} className="relative h-full flex items-center justify-center">
          <div className="bg-white rounded-3xl border border-blue-100 shadow-2xl p-6 flex flex-col items-center justify-center h-full">
            <div
              id="bubble"
              data-anim
              className="opacity-0 px-5 py-1 rounded-full bg-blue-50 border border-blue-200 text-blue-900 font-medium text-sm mb-3 text-center"
            />
            ...
            <div className="flex flex-col items-center gap-2 mt-2 w-full">
              {Array.from({ length: 5 }).map((_, i) => (
                ...
                >
                  ...
                  <span
                    className="text-blue-900 text-sm step-text flex-1"
                    id={`step-text-${i}`}
                  />
                </div>
              ))}
              ...
              return (
                <AnimatePresence mode="popLayout" key={idx}>
                  <motion.div
                    ...
                    className="p-4 rounded-2xl bg-white border border-blue-100 shadow-lg"
                  >
